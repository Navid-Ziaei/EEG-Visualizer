...
def multitaper_spectrogram(data, fs, frequency_range=None, time_bandwidth=5, num_tapers=None, window_params=None,
  ...
  __________________________________________________________________________________________________________________
    ...
    if plotly_on:
        heatmap = plotly_heatmap(mt_spectrogram, stimes, sfreqs)
        fig3 = go.Figure(heatmap)
        #fig3['layout']['yaxis']['autorange'] = "reversed"

        return mt_spectrogram, stimes, sfreqs, fig3

    return mt_spectrogram, stimes, sfreqs


# Helper Functions #
def plotly_heatmap(mt_spectrogram, stimes, sfreqs):
    spect_data = nanpow2db(mt_spectrogram)
    ...
    time_values = np.linspace(stimes[0] - dx, stimes[-1] + dx, spect_data.shape[-1])
    frequency_values = np.linspace(sfreqs[0] - dy, sfreqs[-1] + dy, spect_data.shape[-2])
...
def process_spectrogram_params(fs, nfft, frequency_range, window_start, datawin_size, times=None):
    ...
    if times is None:
        ...
        window_middle_samples = window_middle_samples.astype('int')
        stimes = times[window_middle_samples]
